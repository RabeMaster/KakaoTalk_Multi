...
int main() {
  system("Color 0F");
  SetConsoleTitle("KakaoTalk_Multi");
...
bool patch(DWORD hPid) {
  ...
  _NtQuerySystemInformation = (NtQuerySystemInformation_t)GetProcAddress(
      hInst, "NtQuerySystemInformation");
  _NtQueryObject = (NtQueryObject_t)GetProcAddress(hInst, "NtQueryObject");

  buffer = (char*)calloc(bufferSize, sizeof(char));

  while ((status = _NtQuerySystemInformation(
              (SYSTEM_INFORMATION_CLASS)SystemExtendedHandleInformation, buffer,
              bufferSize, NULL)) == STATUS_INFO_LENGTH_MISMATCH) {
    free(buffer);
    bufferSize *= 2;
    buffer = (char*)calloc(bufferSize, sizeof(char));
  }
  ...
  for (i = 0; i < handles->NumberOfHandles; i++) {
    ...
    if (pEntry->ObjectTypeIndex == 19)  // semapore
    {
      ...
      if (wcsstr(nameBuf + 4, L"97C4DDD9") != 0) {
        HANDLE handle;
        DuplicateHandle(hProcess, (HANDLE)pEntry->HandleValue,
...
std::vector<DWORD> get_list() {
  ...
  do {
    if (!strcmp(processInfo.szExeFile, "KakaoTalk.exe")) {
      temp.push_back(processInfo.th32ProcessID);
      continue;
    }
